...
class project():
    ...
    def process_scrtxt(self, content: dict):
        ...
        # Get current target idx, and add all local vars to it
        #self._get_target()
        ...
        for broadcast_name in self.broadcasts:
            ...
            self.data["targets"][0]["broadcasts"][broadcast_id] = broadcast_name # 0th target it stage
    ...
    def _parse(self, lines: list, substack=False, depth=0):
        ...
        while line_num < len(lines):
            ...
            token_type = command_manager.get_command(token_list["name"], self.id)
    ...
    def _simplify_args(self, args: list | str, itr = 0):
        ...
        for arg in args:
            # Simplify any expressions: e.g. (1 + 2) * 3 --> multiply(add(1,2),3)
    ...
    def _create_block(self, name, args, prev=None, comment=None):
        ...
        data = command_manager.get_command(name, self.id)
